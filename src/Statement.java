...
public class Statement {
	...
	private static final String ACCEPTED_CHARACTERS = NUMBERS + OPERATORS + VARIABLES;
	...
	public float parseStatement(int x, int y, int w, int h) {
		...
		for (int i = 0; i < terms.size(); i++) {
			...
			if(i < terms.size() - 2) isNextImportant = (terms.get(i + 2).charAt(0) == '*') || (terms.get(i + 2).charAt(0) == '*');
